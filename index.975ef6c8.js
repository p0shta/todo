...
try {
    toDoesStorage = [
        ...JSON.parse(localStorage.getItem('to-does'))
    ];
} catch (error) {
    toDoesStorage = [];
}
...
function makeToDoStyleChecked(e) {
    if (e.target.nodeName === 'BUTTON') return;
    const toDoId = e.target.closest('li').id;
    for (const todo of toDoesStorage)if (todo.id === toDoId) {
        todo.checked = !todo.checked;
        toDoApp.updRenderAndAddToLocalStorage();
    }
}

},{"./sass/main.scss":"dFl68"}],"dFl68":[function() {},{}]},["7fmqN","8lqZg"], "8lqZg", "parcelRequire5af9")
